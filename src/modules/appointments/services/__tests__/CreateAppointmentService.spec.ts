...
describe('CreateAppointment', () => {
  beforeAll(() => {
    jest.spyOn(Date, 'now').mockImplementation(() => new Date(2020, 5, 14, 12).getTime());
  });
  ...
  it('should not be able to create an appointment before 8am and after 5pm', async () => {
    await expect(
      createAppointment.execute({ ...newAppointment, date: new Date(2020, 5, 15, 7) }),
    ).rejects.toEqual(new AppError('You can only create appointments between 8am and 5pm.'));

    await expect(
      createAppointment.execute({ ...newAppointment, date: new Date(2020, 5, 15, 18) }),
    ).rejects.toEqual(new AppError('You can only create appointments between 8am and 5pm.'));
  });
});
